...
public class PublishCityScheduler implements Scheduler {

    private Logger log = LoggerFactory.getLogger(getClass());

    @Autowired
    @Qualifier("cityHtmlQueryListServiceImpl")
    private QueryListService cityQueryListService;

    @Autowired
    @Qualifier("cityPublishManager")
    private PublishManager cityPublishManager;
    ...
    public void schedule() {
        ...
        try {
            ...
            log.error("Exception happened during Publish City Scheduler: ", e);
            throw new RuntimeException(e);
        }
    }

    private void publishCity() throws IllegalAccessException, InvocationTargetException, NoSuchMethodException,
            UnsupportedEncodingException, URISyntaxException, InterruptedException {
        publishCityInternal("viking lasten talvi", "TALVIKENGAT", 0);
    }

    private void publishCityInternal(String query, String categoryenum, int limit)
        ...
        Collection<ListResponse> queryListResponses = cityQueryListService.search(query, searchParams);
        ...
        if (category != null) {
            cityPublishManager.publishResults(category, list);
        } else {
